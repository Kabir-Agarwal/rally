# QUEUE

- [x] 1. Item #0: re-study Playeri for steal-worthy features, run the reuse-audit, verify Vercel/Supabase free-tier headroom  (SPEC: Y7)
- [x] 2. Lock prod-safe delivery: block-by-block, dummy UI on new screens  (SPEC: Y1)
- [x] 3. Keep the theme  (SPEC: Y2)
- [x] 4. Keep doubles chemistry  (SPEC: Y2)
- [x] 5. Keep delete-past-matches  (SPEC: Y2)
- [x] 6. Keep earned-from-0 card stats  (SPEC: Y2)
- [x] 7. Add tournaments — 4–32 draws, BYEs, live brackets  (SPEC: Y3)
- [x] 8. Add round-robin leagues  (SPEC: Y3)
- [x] 9. Add practice-vs-rated split  (SPEC: Y3)
- [x] 10. Add competitions hub — Create/Manage/In-Queue/Scheduled  (SPEC: Y3)
- [x] 11. Add nearby players — 50km + Connect  (SPEC: Y3)
- [x] 12. Add messenger + basic AI helper  (SPEC: Y3)
- [x] 13. Add highlights — landscape video upload, reputation stars, laurels  (SPEC: Y3)
- [x] 14. Add posts feed + followed players' matches  (SPEC: Y3)
- [x] 15. Add org/club accounts  (SPEC: Y3)
- [x] 16. Add public layer — privacy controls, block/report, location-permission handling, 16+ gate  (SPEC: Y3)
- [x] 17. Build Elo-style skill rating on the FIFA card  (SPEC: Y4)
- [x] 18. Build activity/reputation form count  (SPEC: Y4)
- [x] 19. Adopt batched migrations — one migrations/block-N.sql per feature-block, owner applies via Supabase MCP  (SPEC: Y5)
- [x] 20. Build Y6 Tier 1 video tool — rally segmentation, counts/lengths, highlight reel, movement heatmaps; laptop-only, upload stats JSON + clips  (SPEC: Y6)
- [x] 21. Gate Y6 Tier 2 on owner's real Stage-0 clip validation  (SPEC: Y6)
